            ...
            <table class="table">
                ...
                <tr v-else v-for="item in items" :key="item.id">
                    ...
                    <td class="text-right">
                        ...
                        <button @click="generate(item.id)" type="button" class="btn btn-ideen" v-else-if="role !== 'admins' && role !== 'personnels' && role !== 'finances'">
    ...
    watch: {
        searchText(val) {
            this.items = this.users.filter(item => {
                ...
                const mothers_lastname = item.mothers_lastname ?? '';
                return !(name.search(val) < 0)
                    || !(lastname.search(val) < 0)
                    || !(mothers_lastname.search(val) < 0);
            })
        },
    }
}
