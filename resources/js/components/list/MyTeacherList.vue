        ...
        <div class="row justify-content-center">
            <table class="table table-striped">
                ...
                <tr v-else v-for="teacher in teachers" :key="teacher.id">
                    ...
                    <td>{{ teacher.email }}</td>
                    <td class="text-right">
                        <button @click="erase(teacher.id)" type="button" class="btn btn-danger">
...
export default {
    ...
    methods: {
        loadCourses() {
            this.isLoading = true
            axios
                .get(this.route + '/teachers/' + this.group)
        ...
        erase: function (id) {
            this.$swal({
                ...
                preConfirm: () => {
                    ...
                    return axios
                        .put(this.route + '/edit/' + this.group + '/detachTeacher', {
                            id
                        })
                        .then(response => {
                            const removedId = this.teachers.findIndex(item => item.id === id);
                            this.teachers.splice(removedId, 1);
                        })
                        .catch(error => {
