<template>
    <form
        ...
        <div class="row">
            <div class="col">
                <h5 class="text-center">Agregar cobro extra</h5>
            </div>
        </div>
        <input type="hidden" name="_token" :value="csrf">

        <div class="form-group row">
            <label for="concept" class="col-md-4 col-form-label text-md-right">Conceptos</label>
            <div class="col-md-6">
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <label class="input-group-text" for="concept">
                    ...
                    >
                        <template #option="{ name, total }">
                            <div class="vs__dropdown-text">{{ name }}<em>{{ Number.parseFloat(total).toFixed(2) }}</em></div>
                        </template>
                        <template #selected-option="{ name, total }">
                            <div style="display: flex; align-items: baseline">
                                <strong>{{ name }}</strong>
                                <em style="margin-left: 0.5rem">${{ Number.parseFloat(total).toFixed(2) }}</em>
                            </div>
                        </template>
                    </v-select>

                    <span
                        v-if="!$v.concept_id.error"
